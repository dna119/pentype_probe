...
public class Texture2dProgram {
    ...
    public void setKernel(float[] values, float colorAdj) {
        if (values == null) {
            throw new IllegalArgumentException("Kernel values array must not be null");
        }
        ...
        for (int i = 0; i < values.length; i++) {
            ...
            if (Float.isNaN(v) || Float.isInfinite(v)) {
                throw new IllegalArgumentException("Kernel value at index " + i + " is invalid: " + v);
            }
            // 선택적 범위 검증 추가
            if (v < -10.0f || v > 10.0f) {
                throw new IllegalArgumentException("Kernel value out of range at index " + i + ": " + v);
            }
        }

        if (Float.isNaN(colorAdj) || Float.isInfinite(colorAdj)) {
            throw new IllegalArgumentException("Invalid colorAdj value: " + colorAdj);
        }
        // 선택적 범위 검증 추가
        if (colorAdj < 0.0f || colorAdj > 2.0f) {
            throw new IllegalArgumentException("colorAdj out of allowed range (0.0~2.0): " + colorAdj);
        }
