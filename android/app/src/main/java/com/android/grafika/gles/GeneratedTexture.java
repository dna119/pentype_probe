...
public class GeneratedTexture {
    ...
    private static ByteBuffer generateCoarseData() {
        ...
        for (int i = 0; i < buf.length; i += BYTES_PER_PIXEL) {
            ...
            final int redInt   = (color & 0xff);
            final int greenInt = ((color >>> 8) & 0xff);
            final int blueInt  = ((color >>> 16) & 0xff);
            final int alphaInt = ((color >>> 24) & 0xff);

            final float redF   = (float) redInt;
            final float greenF = (float) greenInt;
            final float blueF  = (float) blueInt;
            final float alphaF = (float) alphaInt;

            final float alphaM = alphaF / 255.0f;
            buf[i]     = (byte) Math.round(redF * alphaM);
            buf[i + 1] = (byte) Math.round(greenF * alphaM);
            buf[i + 2] = (byte) Math.round(blueF * alphaM);
            buf[i + 3] = (byte) alphaInt;
    ...
    private static void checkerPattern(byte[] buf, int left, int top, int right, int bottom,
        ...
        for (int row = top; row < bottom; row++) {
            ...
            for (int col = left; col < right; col++) {
                ...
                final int redBits   = (color & 0xff);
                final int greenBits = ((color >>> 8) & 0xff);
                final int blueBits  = ((color >>> 16) & 0xff);
                final int alphaBits = ((color >>> 24) & 0xff);

                final float alphaM = (float) alphaBits / 255.0f;
                buf[offset]     = (byte) Math.round((float) redBits * alphaM);
                buf[offset + 1] = (byte) Math.round((float) greenBits * alphaM);
                buf[offset + 2] = (byte) Math.round((float) blueBits * alphaM);
                buf[offset + 3] = (byte) alphaBits;
            }
        }
    }
}
